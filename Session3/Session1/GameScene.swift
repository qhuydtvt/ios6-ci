...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func addEnemies() {
        ...
        let shotAction = SKAction.run {
            ...
            let bulletPosition = enemy.position.add(
                x: 0,
                y: -(enemy.size.height + bulletController.height)/2)
...
//        bullet.run(SKAction.sequence([moveToRoof, SKAction.removeFromParent()]))
        ...
        let startPosition = CGPoint(x: player.position.x,
                                    y: player.position.y +
                                        (player.size.height + bulletController.height) / 2)
